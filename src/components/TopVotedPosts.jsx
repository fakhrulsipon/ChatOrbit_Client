...
const TopVotedPosts = () => {
    const { data: topPosts = [], isLoading, isError, error } = useQuery({
        ...
        queryFn: async () => {
            try {
                ...
                console.warn("Failed to fetch from /top-voted, falling back to /posts?sort=popularity. Error:", err);
                const fallbackRes = await axios.get('https://chatorbit-server.vercel.app/posts?page=1&sort=popularity');
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col justify-center items-center h-80 gap-4">
                ...
                <p className="text-sm font-medium text-slate-500 animate-pulse">Loading top discussions...</p>
            </div>
        );
    }

    if (isError) {
        console.error("TopVotedPosts loading failed with error:", error);
        return (
            <div className="text-center py-10 bg-rose-950/20 border border-rose-900/30 rounded-2xl max-w-md mx-auto my-8">
                <p className="text-rose-400 font-semibold">Failed to load top voted posts</p>
            </div>
        );
    }
                        ...
                        <div>
                            <div className="flex items-center gap-3.5 mb-4">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-[10px] text-slate-400">{new Date(post.postTime).toLocaleDateString(undefined, {month: 'short', day: 'numeric', year: 'numeric'})}</p>
                                </div>
                            </div>
